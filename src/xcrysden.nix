{ lib, fetchurl, fftw, gcc8Stdenv, gfortran, libGL, libGLU, tcl, tk, util-linux
, xorg }:
...
let
  bwidget = rec {
    ...
    src = fetchurl {
      url =
        "http://sourceforge.net/projects/tcllib/files/BWidget/${ver}/${tgz}";
  ...
  togl = tcl.mkTclDerivation rec {
    ...
    src = fetchurl {
      url =
        "https://sourceforge.net/projects/togl/files/Togl/${version}/Togl${version}-src.tar.gz";
    ...
    buildInputs = [ tcl tk.dev libGL xorg.libX11 xorg.libXmu ];
...
in gcc8Stdenv.mkDerivation rec {
  ...
  buildInputs = [ fftw gfortran libGL libGLU tcl tk.dev togl xorg.libXmu ];
